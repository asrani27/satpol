...
class ApiController extends Controller
{
    ...
    public function checkkeluhan($nomor)
    {
        try {
            ...
            $pesan = 'Nomor keluhan ' . $nomor .  ' atas nama ' . '*' . json_decode($data->isi)->name . '*' . ' dengan keluhan ' . '*' . json_decode($data->isi)->complaint . '*' . ' berstatus ' .  '*' . strtoupper($param['status']) .  '*' . 'Link Foto : https://sipadu.banjarmasinkota.go.id/storage/foto/' . $data->file;
            return response()->json($pesan);
        } catch (\Exception $e) {
